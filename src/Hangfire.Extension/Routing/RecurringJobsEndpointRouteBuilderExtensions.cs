        ...
        if (!string.IsNullOrWhiteSpace(routePrefix))
        {
            ...
            if (!string.Equals(options.RoutePrefixNormalized, normalizedRoutePrefix, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException(
                    $"Hangfire.Extension was registered with route prefix '{options.RoutePrefixNormalized}', but API mapping was called with '{normalizedRoutePrefix}'. Configure the route in AddHangfireRecurringJobs and use the same value everywhere.");
            }
        }
